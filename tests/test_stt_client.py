...
async def test_receive_audio_forwarded_to_deepgram():
    ...
    with patch("stt_client.DeepgramClient"):
        ...
        mock_conn.send.assert_called_once_with(b"\x00\x01" * 512)
...
def test_keepalive_enabled_on_client():
    """The Deepgram client must be created with keepalive enabled so the
    websocket survives silence (browser closed / nobody talking) instead of
    dying with NET-0001 every ~10s."""
    import sys
    import importlib
    ...
    with patch.object(_mod, "DeepgramClient") as MockDG:
        _mod.STTClient(api_key="fake", on_transcript=lambda t: None)
        _, kwargs = MockDG.call_args
        config = kwargs["config"]
        assert config.is_keep_alive_enabled()
...
def test_default_language_from_config():
    ...
    fake_dg.DeepgramClientOptions = MagicMock()
